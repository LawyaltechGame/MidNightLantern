...
const BlogCard = ({ post, index }: { post: (typeof POSTS)[number]; index: number }) => (
  ...
  >
    <div className="relative h-56">
      <img src={post.imageUrl} alt="thumbnail" className="absolute inset-0 h-full w-full object-cover" />
      ...
      <div className="relative z-10 h-full p-6 flex items-end">
        <span className="text-5xl drop-shadow-[0_0_12px_rgba(0,0,0,0.6)]">{post.iconEmoji}</span>
      </div>
    </div>
    <div className="p-6">
      <div className="text-xs text-slate-400">{post.date}</div>
      <h3 className="mt-3 text-xl font-extrabold leading-tight tracking-wide text-slate-100">
        {post.title}
      </h3>
      <p className="mt-3 text-sm text-slate-300">{post.excerpt}</p>
      <motion.span whileHover={{ x: 4 }} className="mt-5 inline-flex">
        <Link to={`/blog/${post.id}`} className="inline-flex items-center gap-2 text-sm font-semibold text-cyan-300">
...
const BlogPage = () => {
  return (
    <div className="mx-auto max-w-7xl px-4 py-12">
      ...
      <div className="mt-12">
        <h2 className="text-3xl font-extrabold tracking-wide text-cyan-300/90">Latest Gaming News</h2>
